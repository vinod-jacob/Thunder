...
#define FREE(x) HeapFree(GetProcessHeap(), 0, (x))
        ...
        inline void ReloadAsync(std::function<void()>& callback)
        {
            ...
            if (IsValid() == true) {
                ...
                _channel->ExchangeAsync<InterfacesFetchType>([this, callback](bool success) {
                    ...
                    if (success == true) {
                        _channel->ExchangeAsync<IPAddressFetchType<false>>([this, callback](bool success) {

                             if (success == true) {
                                _channel->ExchangeAsync<IPAddressFetchType<true>>([this, callback](bool success) {
    ...
    /* static */ void AdapterIterator::FlushAsync(std::function<void()> callback)
    {
        networkController.ReloadAsync(callback);
    }
